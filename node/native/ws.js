...
function makeBinFrameHeader(buf) {
...
let server=net.createServer(socket => {
    ...
    if(headers['Connection']!='Upgrade' || headers['Upgrade']!='websocket'){
      ...
      socket.on('data', (data)=>{
        //这里会有粘包的问题，需要拼接起来
        //解析帧结构
        console.log(`recv client data: ${data.byteLength}`);
        let offset = 0;    
        offset += 2;
        ...
        if (payloadLen0 == 126) {
            ...
            payloadLen = data.readUInt32BE(offset,1) << 32 + 
                data.readUInt32BE(offset+4,1);
            offset += 8;
        }
        let maskKey = 0;
        let maskKeys = [];
